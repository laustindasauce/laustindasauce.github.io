<template>
  <div>
    <v-row justify="center" align="center" class="about">
      <v-col cols="5">
    ...
    >
      <v-col sm="11" md="10" lg="10">
        <v-timeline>
          <v-timeline-item v-for="(job, i) in work" :key="i" small>
            ...
            <template v-slot:opposite>
              <span :class="`headline `" v-text="job.dates"></span>
            </template>
            <div class="py-4">
              <h2 :class="`headline font-weight-bold secondary--text mb-4 `">
                {{ job.title }}
              </h2>
              <h4 @click="goToUrl(job.url)" :class="`subtitle-1 link  mb-2 `">
                {{ job.company }}
              </h4>
              <ul v-for="(role, index) in job.roles" v-bind:key="index">
                <li>
                  {{ role }}
                </li>
              </ul>
